# DESC: Ablation 2 — No mask (L_CE + L_CCL only)
# Removes attribution computation. Uses random mask for counterfactual generation.
...
from models.agm.model import generate_counterfactual, filter_counterfactual
...
def train(target_domain, seed, tokenizer):
    ...
    run = wandb.init(
        project='AGM-NLP-2',
        name=f'ablation2_nomask_{target_domain}_seed{seed}',
        config={
            'model': 'agm-ablation2-nomask', 'target_domain': target_domain,
            'seed': seed, 'batch_size': BATCH_SIZE, 'lr': LR,
            'max_length': MAX_LENGTH, 'epochs': EPOCHS, 'lambda2': LAMBDA2,
        }
    )
    ...
    run_ckpt_dir = os.path.join(CKPT_DIR, f'ablation2_nomask_{target_domain}_seed{seed}')
    os.makedirs(run_ckpt_dir, exist_ok=True)
    best_val_f1, patience, patience_count = 0.0, 3, 0

    for epoch in range(EPOCHS):
        model.train()
        train_loss, total_l_ce, total_l_ccl = 0, 0, 0
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # NO attribution, NO L_mask — use random mask instead
            # Random mask: ~25% of tokens (matching tau_high=0.75 proportion)
            random_mask = torch.rand(input_ids.shape, device=DEVICE) > 0.75

            # Generate counterfactual with random mask
            counterfactual_ids = generate_counterfactual(
                input_ids, attention_mask, random_mask, mlm_model, mask_token_id
            )
        ...
        print(f"Epoch {epoch+1}/{EPOCHS} | Loss: {avg_loss:.4f} | L_CE: {avg_l_ce:.4f} | L_CCL: {avg_l_ccl:.4f} | Val F1: {val_f1:.4f}")
        wandb.log({'epoch': epoch+1, 'train_loss': avg_loss, 'l_ce': avg_l_ce,
                   'l_ccl': avg_l_ccl, 'val_f1': val_f1, 'val_acc': val_acc})
...
def main():
    ...
    for target in DOMAINS:
        ...
        for seed in SEEDS:
            ...
            wandb.init(project='AGM-NLP-2', name=f'ablation2_nomask_{target}_seed{seed}_eval',
                       config={'model': 'agm-ablation2-nomask', 'target_domain': target, 'seed': seed})
            results = evaluate_cross_domain(model, target, tokenizer, seed)
            seed_results.append(results)
            wandb.finish()

        wandb.init(project='AGM-NLP-2', name=f'ablation2_nomask_{target}_summary',
                   config={'model': 'agm-ablation2-nomask', 'target_domain': target,
                           'seeds': SEEDS, 'lambda2': LAMBDA2})
        mean_delta = np.mean([r['delta'] for r in seed_results])
        std_delta  = np.std([r['delta']  for r in seed_results])
        print(f"\n--- Ablation 2 (No Mask) Summary for target: {target} ---")
